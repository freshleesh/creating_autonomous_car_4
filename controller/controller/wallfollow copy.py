...
class WallFollowNode(Node):
    ...
    def _compute(self, dt):
        ...
        alpha_r = math.atan(m)  # wall angle
        wall_dist_r = abs(c) / math.sqrt(m**2 + 1)  # perpendicular distance to wall
        ...
        projected_dist_r = wall_dist_r - self.lookahead * math.sin(alpha_r)
        error = self.target_dist - projected_dist_r


        # error = error_r * 0.8 + error_l * 0.2
        ...
        self.get_logger().info(f'Wall angle (alpha): {math.degrees(alpha_r):.2f} deg, Wall distance: {wall_dist_r:.2f} m, Steering: {steer:.2f} rad, Error: {error:.2f} m')
