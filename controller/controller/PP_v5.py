...
class PPNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f'PPNode v5 ready | v_scale={self.v_scale:.2f}  a_lat_max={self.a_lat_max:.1f}'
    ...
    def _compute(self):
        ...
        # Stage 1 — FAR soft cap: sqrt(a_lat * far_factor / kappa_max_far)
        #   preview_n개(8.5m) 앞 최대 kappa → 코너 8m 전부터 부드럽게 감속
        #   직선(kappa≈0)에서는 분모가 작아 cap 매우 높음 → 사실상 제한 없음
        ...
        # Stage 2 — NEAR hard cap: sqrt(a_lat / kappa_max_near)
        #   alat_near_n개(2.5m) 앞 최대 kappa → 코너 직전 최종 속도 상한
        near_indices = [(target_idx + i) % N for i in range(1, self.alat_near_n + 1)]
        ...
        # [v5] Boost — 차량 현재 위치(kappa_near) + 가까운 미래(max_kappa_near)로 직선 판단
        #   FAR 윈도우 50개는 항상 다음 코너를 포함 → boost 조건으로 부적합
        #   kappa_near: 차가 지금 직선에 있는가 (nearest waypoint)
        #   max_kappa_near: 앞 2.5m 이내에 코너가 없는가 (target 기준 near window)
        if abs(kappa_near) < self.straight_kappa and max_kappa_near < self.straight_kappa:
